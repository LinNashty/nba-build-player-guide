...
- 传奇起始赛季使用中性的年份控件；赛季不再承担属性池分类色，避免与位置色或评级色混淆。
...
页面使用居中的 1180px 最大内容壳层，桌面端两侧至少留 24px；手机端收为视口宽减 28px。桌面首屏约 920px，主体任务区以约 116px 的纵向间隔展开；顶部持续吸附五个位置，年代选择只留在传奇择队模块内部，避免两套全局筛选互相打架。
...
**The Recruitment Timeline Rule.** 传奇球队卡先显示“开局选哪队、何时引援谁”的一句话答案；第二层立即放大入队前真实首发五人；第三层再依次展示五个引援节点、新秀季个人奖项概率与最佳节点六人候选。手机端时间轴转为纵向，不使用横向拖动。
