...
**Creative North Star: "荧光战术台"**

这套界面像一张停在比赛暂停时刻的石墨黑战术板：基底深而安静，控制层像磨砂玻璃悬在其上，荧光绿只在需要玩家立刻行动或确认正确答案时亮起。视觉表达服务于高密度决策，不用装饰抢夺球员、属性、权重和推荐结论的注意力。
...
页面使用居中的 1180px 最大内容壳层，桌面端两侧至少留 24px；手机端收为视口宽减 28px。首屏最小高度 850px，主体任务区以约 116px 的纵向间隔展开，固定导航与锚点滚动共同支持长页速查。

桌面端依任务采用三列推荐卡、两列排行榜、三列奖项卡和 390px + 自适应内容的球队查询布局。980px 以下逐步收为两列或单列；720px 以下核心卡片与结果区全部单列，横向位置和属性控件允许滚动，球队按钮压为五列图标网格。复杂表格形态被拆成可扫描卡片、分组行与按需展开的细节。
...
替补、球队 13 项最高和剧情选项都通过原生 `details/summary` 按需展开。剧情行在打开时加强边框，右侧加号旋转 45 度；推荐选项使用荧光绿浅底与边框，其他选项保留深色实体表面。
