...
function TeamSection({ context, position, setPosition, team, setTeam }: any) {
  ...
  const choices = useMemo(() => context.attrs.map((attr: string) => {
    ...
    return { attr, rows: rows.slice(0, 3) };
...
export default function GuideApp({ careerData, legendData }: { careerData: any; legendData: any }) {
  ...
  return <main><nav className="top-nav"><a className="brand" href="#top"><span>传</span><strong>传奇球星攻略</strong></a><div className="desktop-links">{NAV.slice(1, 8).map(([name, href]) => <a key={href} href={href}>{name}</a>)}</div><button className="menu-trigger" onClick={() => setMenuOpen(true)}>目录</button></nav><Hero mode={mode} setMode={setMode} era={era} setEra={setEra} careerData={careerData} legendData={legendData} /><div className="status-dock"><div className="shell"><div className="mode-status"><span>当前</span><strong>{mode === "career" ? "生涯模式" : "传奇模式"}</strong>{mode === "legend" && <b>{era} 时代</b>}</div><PositionTabs value={position} onChange={setPosition} compact /></div></div><div className="quick-nav shell">{NAV.map(([name, href], index) => <a key={href} href={href}><span>{String(index).padStart(2, "0")}</span>{name}</a>)}</div>{mode === "legend" ? <LegendOverview legendData={legendData} era={era} /> : <CareerOverview careerData={careerData} />}<OptimalSection context={context} position={position} setPosition={setPosition} /><TeamSection context={context} position={position} setPosition={setPosition} team={team} setTeam={setTeam} /><RankingsSection context={context} position={position} setPosition={setPosition} /><StorySection context={context} careerData={careerData} position={position} setPosition={setPosition} /><AwardsSection mode={mode} /><ProfileSection mode={mode} /><LegacySection mode={mode} /><TitleLibrary data={careerData} /><DataSection careerData={careerData} legendData={legendData} /><footer><div className="shell"><div><strong>打造我的传奇球星 · 全方位攻略</strong><p>当前数据快照：{cnDate(legendData.meta.extractedAt)}。游戏更新后请以新核验版本为准。</p></div><a href="#top">回到顶部 ↑</a></div></footer><a className="back-top" href="#top" aria-label="回到顶部">↑</a>{menuOpen && <div className="menu-backdrop" onClick={() => setMenuOpen(false)}><aside className="mobile-menu" onClick={(event) => event.stopPropagation()}><header><div><span>页面目录</span><strong>{mode === "career" ? "生涯模式" : `传奇模式 · ${era}`}</strong></div><button onClick={() => setMenuOpen(false)}>关闭</button></header><div>{NAV.map(([name, href], index) => <a key={href} href={href} onClick={() => setMenuOpen(false)}><span>{String(index).padStart(2, "0")}</span>{name}</a>)}</div></aside></div>}<button className="mobile-menu-button" onClick={() => setMenuOpen(true)}><span>目录</span><b>{mode === "career" ? "生涯" : `${era} 传奇`}</b></button></main>;
}
